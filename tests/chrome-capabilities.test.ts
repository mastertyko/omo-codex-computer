...
function initialize(version = APP_SERVER_VERSION): InitializeResponse {
  return {
    userAgent: `omo-codex-computer/${version} (Mac OS 27.0.0; arm64)`,
...
// OMO_CODEX_CHROME_TRUST or persisted trust store never leaks into a test.
function evaluate(
...
describe("evaluateChromeCapabilities", () => {
  ...
  it("trusts additional app-server versions from OMO_CODEX_CHROME_TRUST", async () => {
    const tree = await createPluginTree();
    const env = { OMO_CODEX_CHROME_TRUST: " 0.150.0 , " };
  ...
  it("reads persisted app-server trust from the config store", async () => {
    ...
    await mkdir(join(configHome, "omo-codex-computer"), { recursive: true });
    await writeFile(
      join(configHome, "omo-codex-computer", "trusted-app-servers.json"),
  ...
  it("ignores malformed trust entries without widening trust", async () => {
    ...
    const result = await evaluate(
      ...
      pluginList(tree.root),
      mcp(),
      { OMO_CODEX_CHROME_TRUST: malformed },
    );
    expectUnavailable(result, "unsupported_app_server_version");
  });
  ...
  it("rejects an unverifiable or untrusted app-server version before touching the plugin", async () => {
    ...
    const missing = await evaluate(
      { ...initialize(), userAgent: "omo-codex-computer" },
      pluginList(tree.root),
      mcp(),
    );
