...
export class ChromeRuntime {
  ...
  beginAgent(ctx: ExtensionContext): Promise<void> {
    return this.queue.enqueue(async () => {
      ...
      // OMO may begin another low-level run for retry or continuation before
      // terminal settlement. Repeated starts for the same session and cwd
      // retain the opaque browser identity and any poisoned state.
      if (this.state.kind === "active" || this.state.kind === "poisoned") {
        const { agent } = this.state;
        if (agent.omoSessionId === sessionId && agent.cwd === cwd) return;
   ...
   * current run (cleanup + child stop), then re-arm the same OMO run with a
  ...
  restart(): Promise<void> {
    return this.queue.enqueue(async () => {
      ...
      this.state = {
        ...
        agent: {
          cwd: agent.cwd,
          omoSessionId: agent.omoSessionId,
          identity: {
            sessionId: randomUUID(),
            turnId: randomUUID(),
          },
  ...
  private requireActiveAgent(ctx: ExtensionContext): ActiveAgent {
    ...
    const { cwd, sessionId } = readContextIdentity(ctx);
    if (sessionId !== this.state.agent.omoSessionId) {
      throw new Error("Chrome request does not match the active OMO session");
    }
...
function readContextIdentity(ctx: ExtensionContext): { cwd: string; sessionId: string } {
  ...
  if (!sessionManager || typeof sessionManager.getSessionId !== "function") {
    throw new Error("Chrome requires an active OMO session");
  }
