...
export function logDebug(event: string, data: Record<string, unknown> = {}): void {
  if (process.env.OMO_CODEX_COMPUTER_DEBUG !== "1" && !process.env.OMO_CODEX_COMPUTER_LOG) return;
  ...
  if (process.env.OMO_CODEX_COMPUTER_DEBUG === "1") {
    process.stderr.write(`[omo-codex-computer] ${entry}\n`);
  }

  const logPath = process.env.OMO_CODEX_COMPUTER_LOG;
