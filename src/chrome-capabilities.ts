...
const APP_SERVER_USER_AGENT_PATTERN = /^omo-codex-computer\/([0-9A-Za-z][0-9A-Za-z.+-]{0,63})(?=$|[\s(])/;
...
const UNAVAILABLE_MESSAGES: Readonly<Record<ChromeUnavailableReason, string>> = Object.freeze({
  ...
  unsupported_app_server_version: "Chrome is unavailable because this Codex app-server version has not been validated. Run /codex-computer trust to contract-check and live-probe the installed stack and trust it on this machine, or set OMO_CODEX_CHROME_TRUST to a comma-separated list of app-server versions.",
  ...
  plugin_contract_mismatch: "Chrome is unavailable because the installed Chrome plugin client no longer exposes the automation contract this extension validates. Update omo-codex-computer or report the mismatch.",
});
